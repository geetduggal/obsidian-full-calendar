...
export default class BoxCalendar extends EditableCalendar {
    ...
    get directory(): string {
        return "/"; // Box calendars scan the entire vault
    }
    ...
    containsPath(path: string): boolean {
        // Check if the file at this path has the matching box property
        const file = this.app.getFileByPath(path);
        if (!file) return false;

        const metadata = this.app.getMetadata(file);
        return metadata?.frontmatter?.box === this.value;
    }
    ...
    async getEventsInFile(file: TFile): Promise<EditableEventResponse[]> {
        ...
        // Only include events with matching box property
        if (frontmatter?.box !== this.value) {
            return [];
        }
    ...
    async createEvent(event: OFCEvent): Promise<EventLocation> {
        throw new Error(
            "Box calendar does not support creating events directly. Events must be created with the box property."
        );
    }
    ...
    getNewLocation(
        ...
        if (lineNumber !== undefined) {
            throw new Error("Box calendar cannot handle inline events.");
        }
    ...
    async move(
        ...
        throw new Error(
            "Moving events between box calendars is not supported."
        );
    }
